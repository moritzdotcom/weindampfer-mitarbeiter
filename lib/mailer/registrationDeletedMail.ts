...
export default function sendRegistrationDeletedMail(
  email: string,
  name: string,
  eventName: string,
  eventDate: string,
  ...
  return sendMail({
    to: email,
    subject: `Du wurdest vom Event ${eventName} ausgetragen`,
    text: `Hallo ${name}, du wurdest von dem Event (${eventName} am ${eventDate}) ausgetragen.`,
    html: `<!DOCTYPE html>
<html lang="de">
<head>
  <meta charset="UTF-8" />
  <title>Austragung vom Event ${eventName}</title>
</head>
          ...
          <tr>
            <td style="padding:0 20px 10px;">
              <h1 style="margin:0; font-size:24px; color:#333333; text-align:center;">
                Du wurdest vom Event "${eventName}" ausgetragen
              </h1>
            </td>
          </tr>
          <tr>
            <td style="padding:0 20px;">
              <div style="background-color:#f0f0f0; padding:20px; border-radius:5px; color:#333333; font-size:16px; line-height:1.5;">
                <p>Hallo ${name},</p>
                <p>du wurdest aus der Veranstaltung <strong>${eventName}</strong> am <strong>${eventDate}</strong> ausgetragen.</p>
              </div>
            </td>
          </tr>
          <tr>
